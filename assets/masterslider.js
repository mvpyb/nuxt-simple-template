...
;( function( $ ) {
  ...
  $.removeDataAttrs = function( $target, exclude ) {
	    var i
	        ...
	        var dataAttrs = $target[0].attributes
	    ...
	    for ( i = 0; i < dataAttrsLen; i++ ) {
	        ...
	        if ( attrName.substring( 0, 5 ) === 'data-' && exclude.indexOf( attrName ) === -1 ) {
...
;( function( $ ) {
  ...
  var isPointer 	= window.navigator.pointerEnabled
  var isMSPoiner 	= !isPointer && window.navigator.msPointerEnabled
  var usePointer = isPointer || isMSPoiner
  // Events
  var ev_start = ( isPointer ? 'pointerdown ' : '' ) + ( isMSPoiner ? 'MSPointerDown ' : '' ) + ( isTouch ? 'touchstart ' : '' ) + 'mousedown'
  var ev_move = ( isPointer ? 'pointermove ' : '' ) + ( isMSPoiner ? 'MSPointerMove ' : '' ) + ( isTouch ? 'touchmove ' : '' ) + 'mousemove'
  var ev_end = ( isPointer ? 'pointerup ' : '' ) + ( isMSPoiner ? 'MSPointerUp ' : '' ) + ( isTouch ? 'touchend ' : '' ) + 'mouseup'
  var ev_cancel = ( isPointer ? 'pointercancel ' : '' ) + ( isMSPoiner ? 'MSPointerCancel ' : '' ) + 'touchcancel'
  ...
  p.createStatusObject = function( evt ) {
    var status_data = {}; var temp_x; var temp_y
...
;( function() {
  ...
  p.changeTo = function( value, animate, speed, snap_num, dispatch ) {
    ...
    if ( animate ) {
      ...
      var timeconst = animFrict + ( speed - 20 ) * animFrict * 1.3 / self.options.maxSpeed
  ...
  p.push = function( speed ) {
    ...
    if ( this.options.snapping ) {
      ...
      var end_snap = this.getSnapNum( this.end_loc )
...
;( function( window, document, $ ) {
  ...
  p._mouseParallaxMove = function( e ) {
    var that = e.data.that
    var os = that.slide.$element.offset()
...
( function( $ ) {
  ...
  MSLayerEffects.setup = function() {
    ...
    var transform_css 		= window._jcsspfx + 'Transform'
    var transform_orig_css = window._jcsspfx + 'TransformOrigin'
    var o					= $.browser.opera // Opera sucks :|
...
( function( $ ) {
  ...
  p.create = function() {
    ...
    if ( layerOrigin ) {
      ...
      var hOrigin = layerOrigin.charAt( 1 )
      var offsetX = this.$element.data( 'offset-x' )
      var offsetY = this.$element.data( 'offset-y' )
  ...
  p.locate = function() {
    ...
    var height 		= parseFloat( this.layersCont.css( 'height' ) )
    var factor; var isPosition
  ...
  p._parallaxCalc = function() {
    ...
    var y_def = this._paraY - this._lastParaY
...
( function( $ ) {
  ...
  p.create = function() {
    ...
    if ( $.browser.msie ) { this.$element.on( 'dragstart', function( event ) { event.preventDefault() } ) } // disable native dragging
  }
...
( function( $ ) {
  ...
  p.create = function() {
    ...
    if ( $.browser.msie ) { this.img.on( 'dragstart', function( event ) { event.preventDefault() } ) } // disables native dragging
  }
...
( function( $ ) {
  ...
  p._alignPolicy = function() {
    var h = this.tt.outerHeight( false )
		    var w = Math.max( this.tt.outerWidth( false ), parseInt( this.tt.css( 'max-width' ) ) )
  ...
  p._locateTT = function() {
    ...
    var os2 = this.slide.slider.$element.offset()

    var dist = 50
    var space = 15 //* this.factor;
    ...
    var arrow_w = 15; var // parseInt(this.tt_arrow.css('border-left')) + parseInt(this.tt_arrow.css('border-right')),
...
( function( $ ) {
  ...
  p.onChangeEnd = function() {
    ...
    if ( this.so.preload > 1 ) {
      var loc; var i; var l = this.so.preload - 1; var slide
  ...
  p.__appendSlides = function() {
    var slide; var loc; var i = 0; var l = this.view.slideList.length - 1
  ...
  p.__updateWindowHash = function() {
    ...
    var dl = this.so.deepLink
    var dlt = this.so.deepLinkType
    var eq = dlt === 'path' ? '\/' : '='
    var sep = dlt === 'path' ? '\/' : '&'
    var sliderHash = dl + eq + ( this.view.index + 1 )
    var regTest = new RegExp( dl + eq + '[0-9]+', 'g' )
  ...
  p.setup = function() {
    ...
    var startSlide = slideInHash !== -1 ? slideInHash : this.so.start - 1
...
( function( $ ) {
  ...
  p._shuffleSlides = function() {
    var slides = this.$element.children( '.ms-slide' ); var r
...
;( function( $ ) {
  ...
  p._checkCritMargins = function() {
    ...
    var inView 	= this.viewSlidesList.indexOf( this.slideList[this.index] )
    var size 	= ( this[this.__dimension] + this.spacing )
    var cm 		= this.options.critMargin
  ...
  p.__updateViewList = function() {
    ...
    // update view list
    this.viewSlidesList = []
    var i = 0; var hlf = Math.floor( this.options.viewNum / 2 ); var l
  ...
  p.__locateSlides = function( move, start ) {
    ...
    var l = this.viewSlidesList.length; var slide
  ...
  p.__updateSlidesZindex = function() {
    ...
    if ( this.loop ) {
      ...
      var afterNum = l - beforeNum
      var diff = beforeNum - afterNum
...
( function( $ ) {
  ...
  p.slideAction = function( slide ) {
    ...
    if ( $.browser.msie ) { thumb_ele.on( 'dragstart', function( event ) { event.preventDefault() } ) } // disable native dragging
  ...
  p.setupSwipe = function() {
    ...
    var that = this
    if ( this.options.dir === 'h' ) { this.swipeControl.onSwipe = function( status ) { that.horizSwipeMove( status ) } } else { this.swipeControl.onSwipe = function( status ) { that.vertSwipeMove( status ) } }
  }
...
( function( $ ) {
  ...
  p._draw = function() {
    ...
    this.ctx.beginPath()
    this.ctx.arc( this.__w * 0.5, this.__w * 0.5, this.options.radius, Math.PI * 1.5, Math.PI * 1.5 + 2 * Math.PI * this.prog, false )
...
( function( $ ) {
  ...
  p.moveParallax = function( e ) {
    var that = e.data.that
    var slider = that.slider
    var offset = that.sliderOffset
    var scrollTop = $( window ).scrollTop()
    var layers = that.currentSlide.$scrollParallaxCont
    var out = offset - scrollTop
  ...
  p.updateSlidesBG = function( pos, fixed ) {
    var slides = this.slider.api.view.slideList
    var position = ( fixed && !$.browser.msie && !$.browser.opera ? 'fixed' : '' )
...
( function( document, window, jQuery ) {
  ...
  p.init = function() {
    ...
    var superRef = view.controller.__renderHook.ref
